...
6. The VM must make the repository available at the path configured in `master.cfg` (currently `/Volumes/home_ext1/src_pierre/all_of_sotf`), because the Windows builders use that path as `workdir`. If the VM cannot use that exact path, edit `master.cfg` to set the correct `workdir` for the Windows builders.
